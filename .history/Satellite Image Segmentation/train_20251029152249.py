...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train-image-dir", required=False, default=None)
    parser.add_argument("--train-mask-dir", required=False, default=None)
    ...
    parser.add_argument("--demo", action="store_true", help="Run a quick demo with synthetic data (no files needed)")
...
def main() -> None:
    ...
    if args.demo:
        # Synthetic data for a quick smoke test
        class SyntheticDataset(Dataset):
            ...
            def __getitem__(self, idx):
                ...
                mask = torch.randint(low=0, high=self.num_classes, size=(256,), dtype=torch.long)
                mask = mask.unsqueeze(0).repeat(256, 1).t()  # 256x256 simple pattern
                return image, mask

        train_loader = DataLoader(SyntheticDataset(length=16, num_classes=args.num_classes),
                                  batch_size=args.batch_size, shuffle=True, num_workers=0)
        val_loader = DataLoader(SyntheticDataset(length=8, num_classes=args.num_classes),
                                batch_size=args.batch_size, shuffle=False, num_workers=0)
        model = DeepGlobeModel(num_classes=args.num_classes, use_pretrained=False).to(device)
    else:
        if not args.train_image_dir or not args.train_mask_dir:
            raise SystemExit("--train-image-dir and --train-mask-dir are required unless --demo is used")
